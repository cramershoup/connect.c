 ...
 *
 *   Accepts password from command line options:
 *     https://github.com/cramershoup/connect.c
 *
 * How To Compile
 * ==============
 *
 ...
 *                   [-H [user[:pass]@]proxy-server[:port]]
 *                   [-S [user[:pass]@]socks-server[:port]]
 *                   [-T proxy-server[:port]]
 ...
 *
 *   The -H option specifys a hostname and port number of the http proxy server
 *   to relay. If port is omitted, 80 is used. You can specify this value in the
 *   environment variable HTTP_PROXY and pass the -h option to use it.
 *
 *   The -S option specifys the hostname and port number of the SOCKS server to
 *   relay.  Like -H, port number can be omitted and the default is 1080. You
 ...
 *   indicates protocol version to use. It is valid only when used with '-s' or
 *   '-S'. Default is '-5' (protocol version 5)
 *
 *   The '-R' option is for specifying method to resolve the hostname. Three
 *   keywords ("local", "remote", "both") or dot-notation IP address are
 *   acceptable.  The keyword "both" means, "Try local first, then remote". If a
 *   dot-notation IP address is specified, use this host as nameserver. The
 *   default is "remote" for SOCKS5 or "local" for others. On SOCKS4 protocol,
 *   remote resolving method ("remote" and "both") requires protocol 4a
 *   supported server.
 *
 *   The '-p' option will forward a local TCP port instead of using the standard
 *   input and output.
 *
 *   The '-P' option is same to '-p' except keep remote session. The program
 *   repeats waiting the port with holding remote session without disconnecting.
 *   To disconnect the remote session, send EOF to stdin or kill the program.
 *
 *   The '-w' option specifys timeout seconds for making connection with TARGET
 *   host.
 *
 *   The '-d' option is used for debug. If you fail to connect, use this and
 ...
 *   means this connect-25 command is spcifying port number 25 already so you
 *   need not 2nd argument (and ignored if specified).
 *
 *   To use proxy, this example is for SOCKS5 connection to connect to 'host' at
 *   port 25 via SOCKS5 server on 'firewall' host.
 ...
 * =====================
 *
 *   NOAUTH and USER/PASS authentications are supported.
 *
 ...
 * ===========================
 *
 *   User name for authentication is specifed by spec or from an environment
 *   variable. Password is specified by spec or from an environment
...
#ifndef LINT
static char *vcid = "$Id: connect.c 101 2018-12-06 00:00:00Z gotoh $";
#endif
...
static char *usage =
    ...
    "[-R resolve] [-w timeout] \n"
#endif                /* not _WIN32 */
    "          [-H proxy-server[:port]] [-S [user[:pass]@]socks-server[:port]] \n"
    "          [-T proxy-server[:port]]\n"
    "          [-c telnet-proxy-command]\n"
    "          host port\n";
...
char *relay_user = NULL;             /* username for auth */
char *relay_pass = NULL;             /* password for auth */
...
void debug_(const char *fmt, ...) /* without prefix */
{
    va_list args;
    if (f_debug)
    {
...
void read_parameter_file_1(const char *name)
{
    FILE *f;
    int line;
    char lbuf[1025];
...
int add_direct_host(const char *name, int negative)
{
    if (MAX_DIRECT_ADDR_LIST <= n_direct_addr_list)
    {
        error("direct address table is full!\n");
...
BOOL __stdcall w32_intr_handler(DWORD dwCtrlType)
{
...
int w32_tty_readpass(const char *prompt, char *buf, size_t size)
{
...
/* SOCKS5 and HTTP Proxy authentication may requires username and
   ...
   Username is taken from
     1) server location spec (i.e. user:pass@host:port)
   ...
   Password is taken from
     1) password spec (i.e. user:pass@host:port)
...
/* set_relay()
   ...
   method, host, port, username, and password.
   1st arg, METHOD should be METHOD_xxx.
   2nd arg, SPEC is hostname or hostname:port or user@hostame:port or user:pass@hostname:port.
...
int set_relay(int method, char *spec)
{
    ...
    if (relay_user != NULL)
    {
        sep = strchr(relay_user, ':');
        if (sep != NULL)
        {
            *sep = '\0';
            relay_pass = strdup(spec);
        }
        else
        {
            relay_pass = determine_relay_password();
        }
    }
    else
    {
...
socks5_do_auth_userpass(int s)
{
    ...
    if (relay_pass == NULL &&
        (relay_pass = readpass("Enter SOCKS5 password for %s@%s: ",
    ...
    ptr += len;
    len = strlen(relay_pass); /* PLEN and PASSWD */
    PUT_BYTE(ptr++, strlen(relay_pass));
    strcpy(ptr, relay_pass);
    ptr += len;
    memset(relay_pass, 0, strlen(relay_pass)); /* erase password */
...
/* begin SOCKS protocol 4 relaying
   ...
   1. If server spec has user:pass@hostname:port format then
      user part is used for this SOCKS server.

   2. Get user name from environment variable.
...
int basic_auth(SOCKET s)
{
    ...
    if (relay_pass == NULL &&
        (relay_pass = readpass("Enter proxy authentication password for %s@%s: ",
                               relay_user, relay_host)) == NULL)
        fatal("Cannot decide password for proxy authentication.");

    len = strlen(user) + strlen(relay_pass) + 1;
    userpass = xmalloc(len + 1);
    snprintf(userpass, len + 1, "%s:%s", user, relay_pass);
    memset(relay_pass, 0, strlen(relay_pass));
...
int begin_telnet_relay(SOCKET s)
{
    char buf[1024];
    char *cmd;
...
int do_repeater(SOCKET local_in, SOCKET local_out, SOCKET remote)
{
...
int accept_connection(u_short port)
{
    static int sock = -1;
    int connection;
